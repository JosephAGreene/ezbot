...
const schema = Joi.object({
  ...
  embedColor: Joi.string().trim().regex(RegExp(/^#([0-9a-f]{3}|[0-9a-f]{6})$/i)).max(7).allow('').optional()
    .messages({
...
export default function ModerationHelp(props) {
  ...
  const onSubmit = async (data) => {
    ...
    const res = await ModerationService.updateHelpModeration(payload);
    ...
    if (res.status === 200) {
      ...
      setApiAlert({
        ...
        duration: 2500,
        severity: "success",
        message: "The help module has been udpated!"
      });
